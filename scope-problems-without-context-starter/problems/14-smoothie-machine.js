...
function smoothieMachine(...params){
  const text="I'm having a smoothie with ";
  let sentence="";
  
  let smoothie= (...pears)=>{
    
    if(params.length>0 && pears.length>0){
      params.forEach((str)=>{
        sentence= text+" and "+str
      });
      pears.forEach((str)=>{
        sentence= sentence+" and "+str
      });
    }
    else if(params.length==0 && pears.length==1){
