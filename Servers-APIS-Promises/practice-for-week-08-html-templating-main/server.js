...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    if (req.method === 'GET' && req.url === '/dogs/new') {
      // Your code here
      const newForm = fs.readFileSync("./views/create-dog.html", 'utf-8');
      res.statusCode = 200;
      res.setHeader("Content-Type", "text/html");
     
      return res.end(newForm);
    ...
    if (req.method === 'GET' && req.url.startsWith('/dogs/')) {
      ...
      if (urlParts.length === 4 && urlParts[3] === 'edit') {
        ...
        const templateNew = fs.readFileSync("./views/edit-dog.html", 'utf-8');
      const editDogsPage = templateNew
    ...
    if (req.method === 'POST' && req.url.startsWith('/dogs/')) {
      ...
      if (urlParts.length === 3) {
        ...
        dogs.push(editDog);
        res.statusCode = 302;
        res.setHeader ("Location","/dogs/"+editDog.dogId);
        return res.end();
      }
    }
